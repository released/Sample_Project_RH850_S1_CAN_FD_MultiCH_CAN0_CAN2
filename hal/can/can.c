...
const CAN_RX_RULE_TABLE_T tbl_can_bus_rx_rule_ch0[CAN_RX_RULE_CURRENT_AMOUNT] = 
{
    /*

        {
            ...
            .GAFLRMDP = can_bus_parameter_ch0.CAN_RX_FIFO_BUFFER_NUM,
            .GAFLRMV = A_RECEIVE_BUFFER_IS_USED,   //1: A receive buffer is used.
            .GAFLPTR = 0x5555,  //Receive Rule Label

            .GAFLFDP_x = (0x1<<can_bus_parameter_ch0.CAN_RX_FIFO_BUFFER_NUM),
...

        {
            ...
            RESERVED_BIT,
            can_bus_parameter_ch0.CAN_RX_FIFO_BUFFER_NUM,
            A_RECEIVE_BUFFER_IS_USED,
            0x5555,
            (0x1<<can_bus_parameter_ch0.CAN_RX_FIFO_BUFFER_NUM),
...
const CAN_RX_RULE_TABLE_T tbl_can_bus_rx_rule_ch2[CAN2_RX_RULE_CURRENT_AMOUNT] = 
{
    /*
...
void can_rx_fifo_buf_int_check(CAN_REG_TYP * can,CAN_BUS_HANDLE *p, CAN_RX_FIFO_BUFER_NUMBER_e rfi_number)
{
    ...
    if(CAN_REG_READ(rfsts[rfi_number].UINT32,CAN_REG_BIT8,CAN_REG_LENGTH_8)!=0)
    {
        ...
        p->rx_data_ack.bytes |= (0x1<<rfi_number);
        can_bus_variable_ch0.can_rx_fifo_cnt++;
    }
}
#endif
                        ...
                        unsigned char rule_count)
{
    ...
    for(q=0;q<rule_count;q++)
    {
        ...
        #else
        p->rrt_handle[q].ptr0.bit.RMDP=can_bus_parameter_ch2.CAN_RX_FIFO_BUFFER_NUM;    //Set the receive buffer number to store received message.
        #endif
        ...
        #else
        p->rrt_handle[q].ptr1.bit.FDPx=(0x1<<can_bus_parameter_ch2.CAN_RX_FIFO_BUFFER_NUM);        //Receive FIFO Buffer x Select
    ...
    can_receive_rule_setting(can,(CAN_CHANNEL_SEL_e)can_bus_parameter_ch2.CAN_CH,rule_count);
                        ...
                        unsigned char rule_count)
{
    ...
    can_receive_rule_setting(can,(CAN_CHANNEL_SEL_e)can_bus_parameter_ch0.CAN_CH,rule_count);
...
{    
	can_bus_variable_ch0.can_rx_isr_cnt++;
    ...
    can_bus_variable_ch0.can_rx_flag = 1;
}
...
void can_init(void)
{
    ...
    #if defined (ENABLE_MULTI_CAN_CH)
    can_handle_init(&can_bus_handle_ch2);
    #endif
    ...
    #if defined (ENABLE_MULTI_CAN_CH)
    can_normal_mode_set(&RCFDC0,
    					(CAN_FD_MODE_e)can_bus_parameter_ch2.CAN_MODE,
                        (CAN_CHANNEL_SEL_e)can_bus_parameter_ch2.CAN_CH,
                        (CAN_BIT_TIME_DSJW_SEL_e)can_bus_parameter_ch2.DSJW,              //DSJW
                        (CAN_BIT_TIME_DTSEG2_SEL_e)can_bus_parameter_ch2.DTSEG2,          //DTSEG2
                        (CAN_BIT_TIME_DTSEG1_SEL_e)can_bus_parameter_ch2.DTSEG1,          //DTSEG1
                        (CAN_BIT_TIME_DBRP_SEL_e)can_bus_parameter_ch2.DBRP,              //DBRP
                        (CAN_BIT_TIME_NSJW_SEL_e)can_bus_parameter_ch2.NSJW,              //NSJW
						(CAN_BIT_TIME_NTSEG2_SEL_e)can_bus_parameter_ch2.NTSEG2,          //NTSEG2
						(CAN_BIT_TIME_NTSEG1_SEL_e)can_bus_parameter_ch2.NTSEG1,          //NTSEG1
						(CAN_BIT_TIME_NBRP_SEL_e)can_bus_parameter_ch2.NBRP               //NBRP
                        );
    #endif

    can_rx_buffer_set(&RCFDC0);

    can_rrt_set(&RCFDC0,&can_bus_handle_ch0,tbl_can_bus_rx_rule_ch0,CAN_RX_RULE_CURRENT_AMOUNT);
    #if defined (ENABLE_MULTI_CAN_CH)
    can2_rrt_set(&RCFDC0,&can_bus_handle_ch2,tbl_can_bus_rx_rule_ch2,CAN_RX_RULE_CURRENT_AMOUNT);
    #endif
    ...
    #if defined (ENABLE_MULTI_CAN_CH)
    can_fifo_buffer_set(&RCFDC0,
                        (CAN_RX_FIFO_BUFER_NUMBER_e)can_bus_parameter_ch2.CAN_RX_FIFO_BUFFER_NUM,
                        #if defined (CAN_RX_POLLING)
                        CAN_RX_FIFO_BUFFER_INIERRUPT_DISABLE,   //CAN_RX_FIFO_BUFFER_INIERRUPT_ENABLE
                        #elif defined (CAN_RX_INTERRUPT)
                        CAN_RX_FIFO_BUFFER_INIERRUPT_ENABLE,
    ...
    // can_tx_normal_buffer_set(&can_bus_handle_ch0,can_bus_parameter_ch0.CAN_MODE);

    can_start(&RCFDC0,can_bus_parameter_ch0.CAN_CH);
    #if defined (ENABLE_MULTI_CAN_CH)
    can_start(&RCFDC0,can_bus_parameter_ch2.CAN_CH);
    #endif
    ...
    #if defined (ENABLE_MULTI_CAN_CH)
    can_fifo_buffer_enable(&RCFDC0,can_bus_parameter_ch2.CAN_RX_FIFO_BUFFER_NUM);
    #endif
...
void can_channel_error_probe_after_init(void)
{
    ...
    can_tx_normal_buffer1_set(&can_bus_handle_ch0,
                              can_bus_parameter_ch0.CAN_MODE,
                              p_number,
                              transmit_buffer_id);

    can_tx_buf_data_multi_send(&RCFDC0,
                               &can_bus_handle_ch0.tx_handle[0],
                               can_bus_parameter_ch0.CAN_CH,
                               1U,
                               64U);
    ...
    can_tx2_normal_buffer_set(&can_bus_handle_ch2,
                              can_bus_parameter_ch2.CAN_MODE,
                              p_number,
                              transmit_buffer_id,
                              txbuffer,
                              len);
    ...
    can_tx_buf_data_multi_send(&RCFDC0,
                               &can_bus_handle_ch2.tx_handle[0],
                               can_bus_parameter_ch2.CAN_CH,
                               1U,
                               len);
...
void can_fd_loop_process(void)
{
    ...
    #if defined (ENABLE_MULTI_CAN_CH)
    can_fd_receive_buffer_decode(&RCFDC0,can_bus_parameter_ch2.CAN_RX_FIFO_BUFFER_NUM);
    #endif
    #endif
	...
	if(can_bus_variable_ch2.can_tx4_flag==1)
	{
        ...
        count += 0x10;
        can_tx2_normal_buffer_set(&can_bus_handle_ch2,can_bus_parameter_ch2.CAN_MODE,p_number,transmit_buffer_id,txbuffer,len);
    ...
    if (can_bus_variable_ch0.can_rx_flag)
    {    


        can_bus_variable_ch0.can_rx_flag = 0;
    }
}
